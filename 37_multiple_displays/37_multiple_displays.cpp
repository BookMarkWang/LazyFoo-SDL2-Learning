...
bool init()
{
	...
	else
	{
		...
		if( gTotalDisplays < 2 )
		{
			fprintf(stderr, "Warning: Only one display connected!" );
		}
		...
		for( int i = 0; i < gTotalDisplays; ++i )
		{
			...
			fprintf(stderr, "window index %d, bound(%d,%d,%d,%d)\n", i, gDisplayBounds[i].x, gDisplayBounds[i].y, gDisplayBounds[i].w, gDisplayBounds[i].h);
		}
